...
class NinjaGenFile():
    ...
    def add_meson_package(self, package) -> None:
        self._ninja.build(f"{package.builddir}/build.ninja", "meson_setup",
                          variables={
        ...
        self._ninja.build(f"{package.name}_compile", "meson_compile",
                          implicit=f"{package.name}_setup",
                          variables={"builddir": package.builddir, "name": package.name})
        self._ninja.build(f"{package.name}_install", "meson_install",
                          f"{package.name}_compile",
                          variables={"builddir": package.builddir,
                                     "name": package.name,
